...
export async function generateParagraph(originParagraph: Paragraph): Promise<Docx.ISectionOptions['children']> {
  ...
  switch (paragraphType) {
    case ParagraphType.Mermaid:
      return []
    // return generateMermaid(originParagraph)
...
// async function generateMermaid(originParagraph: Mermaid): Promise<Docx.Paragraph[]> {
//   const { children } = originParagraph
//   // let imageBuffer: Buffer
//   // load from local
//   if (typeof document !== 'object') {
//     // const tempFilePath = `${cwd()}/.temp/mermaid.mmd`
//     // const tempSvgPath = `${cwd()}/.temp/mermaid`
//     // writeFileSync(tempFilePath, children)
//     // await run(tempFilePath, `${tempSvgPath}.svg`)
//     // imageBuffer = readFileSync(`${tempSvgPath}.svg`)
//     // // delete temp file
//     // unlinkSync(tempFilePath)
//     // unlinkSync(`${tempSvgPath}.svg`)
//   }
//   // load from web
//   // else {
//   const { svg } = await mermaid.render('mermaid', children)
//   // const imageBuffer = Uint8Array.from(svg)
//   const encoder = new TextEncoder()
//   const imageBuffer = encoder.encode(svg)
//   // }
...
async function generateCodeBlock(originParagraph: CodeBlock): Promise<Docx.Table[]> {
  ...
  const children = originParagraph.children.map(item => new Docx.Paragraph({ style: StyleId.CodeContent, children: [new Docx.TextRun({ text: item.content })] }))
...
async function generateOrderedList(originParagraph: OrderedList): Promise<Docx.Paragraph[]> {
  ...
  for (const listChild of list) {
    ...
    result.push(new Docx.Paragraph({ style: StyleId.list, numbering, children }))
  }
